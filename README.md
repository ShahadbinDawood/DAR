...
flowchart LR
    ...
    subgraph DAR["Dar — Use Case"]
        direction TB

        UC1(["Manage Home"])
        UC2(["Add Device / Item"])
        UC3(["Track Maintenance"])
        UC4(["Set Maintenance Reminder"])
        UC5(["Add Monthly Bill"])
        UC6(["Track Installments"])
        UC7(["Upload Purchase Invoice"])
        UC8(["Manage Sensors"])
        UC9(["View Sensor Readings"])
        UC10(["Detect Usage Spike"])
        UC11(["Generate AI Report"])
        UC12(["View All Homes Stats"])
        UC13(["View Notification"])
        ...
        A1(["Manage Users"])
        A2(["Manage Subscription Plans"])
        A3(["View Payments"])
        A4(["Send Weather Alert"])
        A5(["Send Smart Alert Intro"])
    end

    subgraph RightActors[" "]
        direction TB
        Admin((Admin))
        System((System<br>Auto))
    end
    ...
    Owner ~~~ DAR ~~~ RightActors

    classDef actor fill:#ffffff,color:#765345,stroke:#765345,stroke-width:2px;
    classDef usecase fill:#ffffff,color:#333333,stroke:#A68972,stroke-width:1.5px;
    classDef admincase fill:#ffffff,color:#333333,stroke:#765345,stroke-width:1.5px;
    classDef invisible fill:#ffffff00,stroke:#ffffff00,color:#ffffff00;

    class Owner,Admin,System actor;
    class UC1,UC2,UC3,UC4,UC5,UC6,UC7,UC8,UC9,UC10,UC11,UC12,UC13,UC14,UC15,UC16,UC17 usecase;
    class A1,A2,A3,A4,A5 admincase;
    class RightActors invisible;
```
---
